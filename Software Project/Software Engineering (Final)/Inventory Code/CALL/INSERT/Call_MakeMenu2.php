...
if(isset($_POST['submit'])){
    ...
    if(empty($data_missing)){
			$query = "Select * FROM inventory  WHERE ID = $t_INGR ORDER BY ID";
			$response = @mysqli_query($dbc, $query);
			$row = mysqli_fetch_array($response);
			if(mysqli_num_rows($response) >0 && $row['CAT'] == 'INGR'){
...
{
echo "Enter valid ID of the Item you want to add <br/>";
}
				}
	else{
...
<?php include ('Call_Ingredients.php');?>
</body>
</html>